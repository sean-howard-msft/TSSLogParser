...
BEGIN
	...
	BEGIN TRAN
	BEGIN TRY
	UPDATE [internal].[master_properties] SET property_value = @MachineName  WHERE property_name = 'MACHINE_NAME'
	UPDATE [internal].[master_properties] SET property_value = @MachineIP  WHERE property_name = 'MACHINE_IP'
	UPDATE [internal].[master_properties] SET property_value = @MasterServicePort  WHERE property_name = 'MASTER_SERVICE_PORT'
	UPDATE [internal].[master_properties] SET property_value = @SSLCertThumbprint  WHERE property_name = 'SSLCERT_THUMBPRINT'
	UPDATE [internal].[master_properties] SET property_value = @MasterHeartbeatIntervalInMs  WHERE property_name = 'MASTER_HEARTBEAT_INTERVALINMS'
