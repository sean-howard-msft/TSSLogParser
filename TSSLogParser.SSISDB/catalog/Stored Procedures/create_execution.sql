...
AS
    ...
    BEGIN
        SET @runinscaleout = 0
        IF (EXISTS(SELECT * FROM [catalog].[catalog_properties] WHERE [property_name] = 'IS_SCALEOUT_ENABLED' AND [property_value]='TRUE')
            AND EXISTS(SELECT * FROM [catalog].[catalog_properties] WHERE [property_name] = 'DEFAULT_EXECUTION_MODE' AND [property_value]='1'))
        BEGIN
            SET @runinscaleout = 1
        END
    END
